...
\emph{Question:} Given the MNIST images, can we easily figure out that
there are 10 separate classes, without labels?  I presume that there
are clustering algorithms which can do this, but I must admit that I
don't know the answer.

\emph{Question:} Learning from a single training instance.  Suppose we
do unsupervised feature learning with images.  Then we see a single
``9''.  Can we use that single 9 to learn to recognize future 9s?

\emph{Question:} Why not use hybrid optimization methods: say,
momentum-based stochastic gradient descent for the initial transient
phase, followed by some other method for the final fine-tuning?

\emph{Question (iterated NAG method):} Why not modify the NAG
optimization method by doing a double iteration?  Start from position
$w$.  Compute a trial value, $\delta w = v$.  Then compute the
gradient at the updated trial position, $w+\Delta w$.  Then do a trial
update to the velocity, based on the gradient at the updated trial
position.  Then use that trial velocity to compute a second trial
update to the position.  Then do a second trial update to the
velocity.  At this point we could terminate.  Or we do more
iterations.  (Does the iterative process converge?)  Good ways to
analyse this: look at a quadratic cost function; look at a
fourth-order cost function; do some simple experiments.

\emph{Question:} What happens if we initialize our weights so that the
graph has the structure of a DAG expander?  This seems likely to have
some advantages.

\emph{Idea: The true cost function.}  Analyse deviations using central
limit theorem.

\emph{Idea: Analyse the expected outputs from subsequent layers of a
  neural network.}  Are they being damped or increased?

\emph{Idea:} Combine sparse initialization with dropout.

\emph{Idea:} Write an analysis of sparse initialization.
